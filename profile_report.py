...
def _dominant_band(profile, x_width: float, x_unit: str) -> tuple[str, float]:
    ...
    if matrix.empty or not np.isfinite(values).any() or not (values > 0).any():
...
def _draw_profile_heatmap(
    ...
    """Draw the fixed-bin speed/draught or power/draught matrix."""
    ...
    for row in range(n_rows):
        for column in range(n_columns):
            ...
            pdf.setFillColor(
                colors.Color(*(channel / 255 for channel in rgb))
            )
            ...
            if np.isfinite(value) and value > 0:
                # Cell values are percentages. The caption supplies the unit.
                label = f"{value:.1f}"
                if value < 0.05:
                    label = "<.1"
                ...
                font_size = min(
                    4.8,
                    (cell_width - 1.0) / label_width_at_one_point,
                )

                pdf.setFont(font, font_size)
                pdf.setFillColor(
                    colors.white if ratio > 0.53 else NAVY
                )
    ...
    # Outline the most frequent operating condition.
    ...
    for row, label in enumerate(matrix.index):
        pdf.drawRightString(
            plot_x - 3,
            plot_top - (row + 0.5) * cell_height - 2,
            str(label),
        )
    ...
    for column in range(0, n_columns, label_step):
        ...
        number = (
            f"{label / 1000:g}k"
            if band_width >= 1000 and label >= 1000
            else f"{label:g}"
        )
        pdf.drawCentredString(
            plot_x + (column + 0.5) * cell_width,
    ...
    pdf.drawCentredString(
        plot_x + plot_width / 2,
        plot_top - plot_height - 17,
        x_axis_title,
    )

    main_band, main_share = _dominant_band(
        profile,
        band_width,
        "kW" if band_width >= 100 else "kn",
    )
...
def _draw_monthly_line_chart(
    ...
    for index, value in enumerate(values):
        ...
        point_x = plot_x + (
            plot_width * index / max(count - 1, 1)
        )
        point_y = plot_y + (
            plot_height * (value - y_min) / (y_max - y_min)
        )
...
def build_a4_profile_report(
    *,
    ...
    """Return a one-page A4 PDF with the assessment results."""
    buffer = io.BytesIO()
    width, height = A4
    pdf = canvas.Canvas(
        buffer,
    ...
    pdf.setTitle(
        f"{vessel_name} Operating Profile and Illustrative Retrofit Payback"
    )
    pdf.setAuthor(
        prepared_by or "Vessel Operating Profile App"
    )
    ...
    pdf.drawString(
        margin,
        height - 13 * mm,
        "Vessel Operating Profile and Illustrative Retrofit Payback",
    )
    ...
    pdf.drawString(
        margin,
        meta_y,
        vessel_name or "Vessel name not available",
    )
    ...
    pdf.drawString(
        margin,
        meta_y - 13,
        f"IMO: {imo_number.strip() or 'Not provided'}",
    )
    pdf.drawString(
        margin + 155,
        meta_y - 13,
    ...
    if prepared_by.strip():
        pdf.drawRightString(
            width - margin,
            meta_y,
            f"Prepared by: {prepared_by.strip()[:55]}",
        )

    total_hours = float(
        overall.get("total_hours", 0.0) or 0.0
    )
    analysis_days = (
        total_hours / 24
        if total_hours > 0
    ...
    card_gap = 6
    card_width = (
        content_width - 3 * card_gap
    ) / 4

    card_values = [
        (
            "Elapsed reporting span",
            _number(analysis_days, 1, " days"),
        ),
        (
            "Noon reports loaded",
            f"{int(noon_records):,}",
        ...
        (
            "M/E propelling hours",
            _number(
                overall.get("propelling_hours"),
                1,
                " h",
            ),
        ),
    ...
    _draw_section_title(
        pdf,
        "Speed-draught and engine power-draught profiles",
        margin,
        section_y,
    )
    ...
    operating_rows = [
        (
            "Highest interval-average STW",
            _number(
                overall.get("max_noon_speed_knots"),
                2,
                " kn",
            ),
        ),
        (
            "Highest reported main-engine power",
            _number(
                overall.get("max_noon_me_output_kw"),
                0,
                " kW",
            ),
        ),
    ...
    table_bottom = _draw_two_column_rows(
        pdf,
        operating_rows,
        margin,
        section_y - 13,
    ...
    heatmaps = [
        ...
        (
            power_profile,
            margin + heatmap_width + heatmap_gap,
            heatmap_width,
            "M/E power / mean draught",
            "M/E power (kW)",
            1000.0,
        ),
    ]
    ...
    peaks = [
        float(
            np.nanmax(
                item[0].percent.to_numpy(dtype=float)
            )
        )
        for item in heatmaps
        if not item[0].percent.empty
        and np.isfinite(
            item[0].percent.to_numpy(dtype=float)
        ).any()
    ]
    ...
    caption = (
        "Cell numbers = % eligible propelling hours; "
        "amber = most frequent condition."
    )
    power_columns = power_profile.percent.columns
    max_reported_power = float(
        overall.get(
            "max_noon_me_output_kw",
            float("nan"),
        )
    )
    ...
    if len(power_columns) and math.isfinite(max_reported_power):
        upper_power_bin = (
            float(power_columns[-1]) + 1000
        )
        if max_reported_power >= upper_power_bin:
            caption += (
                f" Power map ends at <{upper_power_bin:,.0f} kW; "
    ...
    if stringWidth(
        ...
        raise ValueError(
            "Heatmap range note is too long for the A4 report"
        )
    pdf.drawString(
        margin,
        heatmap_top - 152,
    ...
    # Monthly graphs appear before the fuel and commercial sections.
    ...
    _draw_monthly_line_chart(
        ...
        "Monthly propelling hours / elapsed hours",
        "%",
    )
    ...
    equivalent_fuel = float(
        fuel.get(
            "total_vlsfo_equivalent_mt",
            0.0,
        ) or 0.0
    )
    raw_fuel = float(
    ...
    if (
        payback
        and payback.get("annual_gross_saving_usd") is not None
    ):
        annual_cost_saving = float(
            payback["annual_gross_saving_usd"]
        )

    section_y = table_bottom - 18
    _draw_section_title(
        pdf,
        "M/E fuel consumption and assumed saving",
        margin,
        section_y,
    )
    ...
    fuel_rows = [
        (
            "Reported M/E fuel, all grades",
            _number(raw_fuel, 2, " t"),
        ),
        (
            "M/E fuel, VLSFO-energy equivalent",
            _number(equivalent_fuel, 2, " t"),
        ),
        (
            "Assumed FOC Saving Reduction",
            _number(period_saving, 2, " t equiv."),
        ),
        (
            "Illustrative annual fuel-cost saving",
            f"US$ {_number(annual_cost_saving, 0)}/year",
        ),
    ]
    ...
    if payback:
        ...
        payback_text = (
            ...
            if (
                payback_value is not None
                and pd.notna(payback_value)
            )
        ...
        unrecovered = float(
            payback.get(
                "unrecovered_capex_usd",
                0.0,
            ) or 0.0
        )
        duration = _number(
            payback.get("charter_duration_years"),
            0,
            " years",
        )
        ...
        commercial_rows = [
            (
                "Project CAPEX",
                f"US$ {_number(payback.get('capex_usd'), 0)}",
            ),
            (
        ...
        commercial_rows = [
            ...
            (
                "Action required",
                "Enter valid CAPEX and saving assumptions in the Payback tab",
            ),
        ]
    ...
    if table_bottom < 42:
        raise ValueError(
            "A4 report content would overlap the footer"
        )

    if management_comment.strip():
        _draw_wrapped(
            pdf,
            f"Comment: {management_comment.strip()}",
            margin,
            39,
            content_width,
            font=FONT_ITALIC,
    ...
    pdf.drawString(
        margin,
        24,
        "Generated by Vessel Operating Profile and Commercial Assessment",
    )
    pdf.drawRightString(
        width - margin,
        24,
